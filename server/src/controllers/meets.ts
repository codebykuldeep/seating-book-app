...
export async function getAllUserMeetings(employee:Employees,date:string) {
    const data = await MeetsLog.find({
        where:{
            booked_by:employee,
...
export async function deleteMeeting(employee:Employees,meet_id:string){
    await MeetsLog.delete(meet_id);
}
